...
export default function Timesheet() {
  ...
  return (
    <div className="timesheet-page">
      <PageHeader
        title="Timesheet"
        description="Registra le ore lavorate. Ogni riga entra subito nei KPI come Approvato."
        actions={
          <>
      ...
      <section className="timesheet-hero">
        <div>
          ...
          <p>Vista operativa delle ore approvate.</p>
        </div>
      ...
      <section className="timesheet-filter-panel">
        ...
        <div className="ts-filter-grid">
          <label><span>Mese</span><input className="input small" type="number" min={1} max={12} value={month} onChange={(e) => setMonth(Number(e.target.value))} /></label>
          <label><span>Anno</span><input className="input small" type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} /></label>
          <label><span>Area</span><Select value={areaFilter} onChange={setAreaFilter} placeholder="Tutte le aree" options={areas.map((a) => ({ value: a.id, label: `${a.codice_area} · ${a.nome_area}` }))} /></label>
          <label><span>Controllo</span><select className="input" value={contestFilter} onChange={(e) => setContestFilter(e.target.value as ContestFilter)}><option value="all">Tutte le righe</option><option value="clean">Solo non contestate</option><option value="contested">Solo contestate</option></select></label>
        </div>
      </section>
                ...
                <div className="ts-card-main">
                  ...
                  <div className="ts-entry-body">
                    <div className="ts-entry-topline">
                      <div><span className="eyebrow">{row.employee_email}</span><h3>{row.employee_name}</h3></div>
                      <div className="ts-status-stack"><span className="status-pill approvato">Approvato</span>{row.is_contested && <span className="status-pill da-correggere">Contestata</span>}</div>
                    </div>
                    <div className="ts-flow">
                      <div><span>Da società</span><strong>{row.employer_company_code ?? "—"}</strong><small>{row.employer_company_name ?? ""}</small></div>
                      <div className="ts-flow-arrow">→</div>
                      <div><span>A società</span><strong>{row.beneficiary_company_code ?? "—"}</strong><small>{row.beneficiary_company_name ?? ""}</small></div>
                    </div>
        ...
        <div className="modal-backdrop ts-modal-backdrop">
          <div className="modal large pro-modal ts-modal">
            <div className="modal-header ts-modal-header">
              ...
              <button className="icon-button" onClick={() => setForm(null)}><X size={18} /></button>
            </div>
            <div className="ts-flow-preview">
              <div><Building2 size={18} /><span>Società datrice</span><strong>{formEmployerCompany?.codice_societa ?? "Da dipendente"}</strong><small>{formEmployerCompany?.ragione_sociale ?? ""}</small></div>
              <div className="ts-flow-preview-arrow">→</div>
              <div><Building2 size={18} /><span>Società beneficiaria</span><strong>{formBeneficiaryCompany?.codice_societa ?? "Da selezionare"}</strong><small>{formBeneficiaryCompany?.ragione_sociale ?? ""}</small></div>
            ...
            <div className="ts-form-sections">
              <section className="ts-form-section">
                <div className="ts-section-head"><UserRound size={18} /><div><h4>1. Chi ha lavorato</h4></div></div>
              ...
              <section className="ts-form-section">
                <div className="ts-section-head"><Layers3 size={18} /><div><h4>2. Dove imputare le ore</h4></div></div>
                <div className="form-grid refined">
              ...
              <section className="ts-form-section">
                <div className="ts-section-head"><FileText size={18} /><div><h4>3. Cosa è stato fatto</h4></div></div>
                <div className="form-grid refined">
                  ...
                  <label className="full">Descrizione lavoro svolto<textarea className="input textarea-large" value={form.descrizione} onChange={(e) => setForm({ ...form, descrizione: e.target.value })} /></label>
                  <label className="full">Note<textarea className="input" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} /></label>
                  {form.id && canManage && <label className="full">Nota correzione<textarea className="input" value={form.correction_note} onChange={(e) => setForm({ ...form, correction_note: e.target.value })} /></label>}
                </div>
              </section>
            </div>
            <div className="modal-actions ts-modal-actions">
              <button className="button secondary" onClick={() => setForm(null)}>Annulla</button>
              <button className="button" onClick={() => void save()} disabled={saving}><Save size={16} /> {saving ? "Salvataggio..." : "Salva ore approvate"}</button>
            </div>
          </div>
        </div>
      )}
...
function StatCard({ icon, label, value, hint, tone = "default" }: { icon: React.ReactNode; label: string; value: string; hint: string; tone?: "default" | "warning" | "success" }) {
  return (<div className={`ts-stat-card ${tone}`}><div className="ts-stat-icon">{icon}</div><span>{label}</span><strong>{value}</strong><small>{hint}</small></div>);
}

function Metric({ label, value }: { label: string; value: string }) {
  return (<div className="ts-metric"><span>{label}</span><strong>{value}</strong></div>);
}
...
function Select({ value, onChange, options, placeholder = "Seleziona" }: { value: string; onChange: (value: string) => void; options: { value: string; label: string }[]; placeholder?: string }) {
  return (<select className="input" value={value ?? ""} onChange={(e) => onChange(e.target.value)}><option value="">{placeholder}</option>{options.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select>);
}
